...
class FinancialAuditor:
    ...
    def _prepare_visualization_data(self, summary_stats: Dict,
        ...
        departments = summary_stats.get('department_distribution', {})
        visualization['department_distribution'] = [
            {"name": dept, "value": round(amount, 2)}
            for dept, amount in sorted(departments.items(), key=lambda x: x[1], reverse=True)
        ]

        # 3. Spending trend (by date)
